...
def select_hub_pair_with_lookup(
    indices, data, rng_state, in_degree, n_hub_candidates=16
):
    """Select two high-degree points that are geometrically distant.

    Optimized version that accepts pre-computed in_degree array to avoid
    redundant computation when called multiple times per split.

    Strategy:
    1. Sample n_hub_candidates points weighted by degree
    2. Pick the pair with maximum geometric distance
    """
...
def select_hub_pair(indices, neighbor_indices, data, rng_state, n_hub_candidates=16):
    ...
    return select_hub_pair_with_lookup(
        indices, data, rng_state, in_degree, n_hub_candidates
    )
...
def generate_and_evaluate_candidate(
    ...
    """Generate a hub pair and evaluate the candidate hyperplane.

    This function is designed to be called in parallel for multiple candidates.
    Each call uses its own RNG state for thread safety.
    Returns (edge_cuts, n_left, n_right, hyperplane_vector, hyperplane_offset, side)
    """
    # Select hub pair using thread-local RNG
    left, right = select_hub_pair_with_lookup(
        indices, data, local_rng_state, in_degree, n_hub_candidates
    )

    # Compute the normal vector to the hyperplane
    hyperplane_offset = np.float32(0.0)
    hyperplane_vector = np.empty(dim, dtype=np.float32)
    ...
    for i in range(n_points):
        ...
        if margin > EPS:
            ...
            # For points exactly on hyperplane, assign based on index parity
            # (deterministic to avoid race conditions in parallel)
            side[i] = i % 2
            if side[i] == 0:
                n_left += 1
            else:
    ...
    # If all points end up on one side, return invalid result
    if n_left == 0 or n_right == 0:
        return (
    ...
    return edge_cuts, n_left, n_right, hyperplane_vector, hyperplane_offset, side
...
def euclidean_hub_split(data, indices, neighbor_indices, rng_state, n_candidates=10):
    ...
    - idx_to_pos and in_degree computed once and reused across candidates
    - Fully parallel: both hub pair generation AND evaluation run in parallel
    - Each thread uses independent RNG state for thread safety
    """
    dim = data.shape[1]
    n_points = indices.shape[0]

    # Build lookup: point index -> position in indices array (-1 if not in node)
    # This is now computed ONCE and reused for all candidates
    idx_to_pos = np.full(neighbor_indices.shape[0], -1, dtype=np.int32)
    for i in range(n_points):
        idx_to_pos[indices[i]] = i

    # Compute in-degree ONCE for hub selection across all candidates
    in_degree = np.zeros(n_points, dtype=np.int32)
    for i in range(n_points):
        point_idx = indices[i]
        for j in range(neighbor_indices.shape[1]):
    ...
    # Create independent RNG states for each candidate (for thread safety)
    # Each state is seeded from the main RNG state with a unique offset
    candidate_rng_states = np.empty((n_candidates, 3), dtype=np.int64)
    for c in range(n_candidates):
        candidate_rng_states[c, 0] = rng_state[0] + c * 1000003
        candidate_rng_states[c, 1] = rng_state[1] + c * 999983
        candidate_rng_states[c, 2] = rng_state[2] + c * 999979

    # Storage for parallel results
    all_edge_cuts = np.full(n_candidates, np.uint32(0xFFFFFFFF), dtype=np.uint32)
    all_n_left = np.zeros(n_candidates, dtype=np.uint32)
    all_n_right = np.zeros(n_candidates, dtype=np.uint32)
    all_hyperplanes = np.zeros((n_candidates, dim), dtype=np.float32)
    all_offsets = np.zeros(n_candidates, dtype=np.float32)
    all_sides = np.zeros((n_candidates, n_points), dtype=np.int8)

    # Fully parallel loop: generate hub pairs AND evaluate candidates in parallel
    for c in numba.prange(n_candidates):
        # Each thread gets its own RNG state
        local_rng = candidate_rng_states[c].copy()

        edge_cuts, n_l, n_r, hp_vec, hp_off, side = generate_and_evaluate_candidate(
            data,
            indices,
            neighbor_indices,
            idx_to_pos,
            ...
            n_points,
            dim,
            16,  # n_hub_candidates=16
        )

        all_edge_cuts[c] = edge_cuts
        all_n_left[c] = n_l
        all_n_right[c] = n_r
        all_offsets[c] = hp_off
        for d in range(dim):
            all_hyperplanes[c, d] = hp_vec[d]
        for i in range(n_points):
            all_sides[c, i] = side[i]

    # Find best candidate (sequential reduction)
    best_idx = 0
    best_edge_cuts = all_edge_cuts[0]
    for c in range(1, n_candidates):
        if all_edge_cuts[c] < best_edge_cuts:
            best_edge_cuts = all_edge_cuts[c]
            best_idx = c

    best_n_left = all_n_left[best_idx]
    best_n_right = all_n_right[best_idx]
    best_hyperplane = all_hyperplanes[best_idx].copy()
    best_offset = all_offsets[best_idx]
    best_side = all_sides[best_idx].copy()

    # Advance the main RNG state to maintain reproducibility
    for _ in range(n_candidates):
        tau_rand_int(rng_state)
...
def generate_and_evaluate_angular_candidate(
    data,
    indices,
    neighbor_indices,
    idx_to_pos,
    ...
    """Generate a hub pair and evaluate the angular candidate hyperplane.

    This function is designed to be called in parallel for multiple candidates.
    Each call uses its own RNG state for thread safety.
    Returns (edge_cuts, n_left, n_right, hyperplane_vector, side)
    """
    # Select hub pair using thread-local RNG
    left, right = select_hub_pair_with_lookup(
        indices, data, local_rng_state, in_degree, n_hub_candidates
    ...
    # If all points end up on one side, return invalid result
    if n_left == 0 or n_right == 0:
        return np.uint32(0xFFFFFFFF), n_left, n_right, hyperplane_vector, side
    ...
    return edge_cuts, n_left, n_right, hyperplane_vector, side
...
def angular_hub_split(data, indices, neighbor_indices, rng_state, n_candidates=10):
    """Hub-based graph-informed angular split.

    Uses high-degree nodes (hubs) to generate candidate hyperplanes,
    then selects the one that minimizes edge cuts.
    Uses angular (cosine) distance for hyperplane computation.
    """
    dim = data.shape[1]
    n_points = indices.shape[0]
    ...
    # Compute in-degree ONCE for hub selection
    in_degree = np.zeros(n_points, dtype=np.int32)
    for i in range(n_points):
        point_idx = indices[i]
        for j in range(neighbor_indices.shape[1]):
    ...
    # Create independent RNG states for each candidate
    candidate_rng_states = np.empty((n_candidates, 3), dtype=np.int64)
    for c in range(n_candidates):
        candidate_rng_states[c, 0] = rng_state[0] + c * 1000003
        candidate_rng_states[c, 1] = rng_state[1] + c * 999983
    ...
    for c in numba.prange(n_candidates):
        ...
        edge_cuts, n_l, n_r, hp_vec, side = generate_and_evaluate_angular_candidate(
            data,
            indices,
            neighbor_indices,
            idx_to_pos,
...
def select_hub_pair_bit_with_lookup(
    indices, data, rng_state, in_degree, n_hub_candidates=16
):
    """Select two high-degree points that are geometrically distant (for bit-packed data).

    Uses Hamming distance for bit-packed data.
    """
    n_points = indices.shape[0]
    dim = data.shape[1]
    ...
    # Find pair with maximum Hamming distance
    best_left = 0
    best_right = 1 if n_hubs > 1 else 0
    ...
    for i in range(n_hubs):
        for j in range(i + 1, n_hubs):
            ...
            dist = np.float32(0.0)
            for d in range(dim):
                dist += popcnt[data[left_idx, d] ^ data[right_idx, d]]
...
def generate_and_evaluate_bit_candidate(
    data,
    indices,
    neighbor_indices,
    idx_to_pos,
    ...
    """Generate a hub pair and evaluate the bit-packed candidate hyperplane."""
    # Select hub pair
    left, right = select_hub_pair_bit_with_lookup(
        indices, data, local_rng_state, in_degree, n_hub_candidates
    )

    # Compute hyperplane for bit data (similar to angular_bitpacked_random_projection_split)
    hyperplane_vector = np.empty(dim * 2, dtype=np.uint8)
    positive_hyperplane_component = hyperplane_vector[:dim]
    negative_hyperplane_component = hyperplane_vector[dim:]
...
def bitpacked_hub_split(data, indices, neighbor_indices, rng_state, n_candidates=10):
    """Hub-based graph-informed split for bit-packed data.
    ...
    # Storage for results (hyperplane is 2*dim for bit data)
    all_edge_cuts = np.full(n_candidates, np.uint32(0xFFFFFFFF), dtype=np.uint32)
    all_n_left = np.zeros(n_candidates, dtype=np.uint32)
    all_n_right = np.zeros(n_candidates, dtype=np.uint32)
    all_hyperplanes = np.zeros((n_candidates, dim * 2), dtype=np.uint8)
    all_sides = np.zeros((n_candidates, n_points), dtype=np.int8)
    ...
    for c in numba.prange(n_candidates):
        ...
        edge_cuts, n_l, n_r, hp_vec, side = generate_and_evaluate_bit_candidate(
            data,
            indices,
            neighbor_indices,
            idx_to_pos,
...
def sparse_select_hub_pair_with_lookup(
    indices, inds, indptr, data, rng_state, in_degree, n_hub_candidates=16
):
    """Select two high-degree points that are geometrically distant (for sparse data).

    Uses squared Euclidean distance for sparse data.
    """
    n_points = indices.shape[0]
    ...
    # Find pair with maximum squared distance
    best_left = 0
    best_right = 1 if n_hubs > 1 else 0
    best_dist_sq = np.float64(0.0)
    ...
    for i in range(n_hubs):
        for j in range(i + 1, n_hubs):
            ...
            left_inds = inds[indptr[left_idx] : indptr[left_idx + 1]]
            left_data = data[indptr[left_idx] : indptr[left_idx + 1]]
            right_inds = inds[indptr[right_idx] : indptr[right_idx + 1]]
            right_data = data[indptr[right_idx] : indptr[right_idx + 1]]

            # Compute squared Euclidean distance via diff then norm
            diff_inds, diff_data = sparse_diff(
                left_inds, left_data, right_inds, right_data
            )
            dist_sq = np.float64(0.0)
            for d in range(diff_data.shape[0]):
                dist_sq += diff_data[d] * diff_data[d]
...
def sparse_euclidean_hub_split(
    inds, indptr, data, indices, neighbor_indices, rng_state, n_candidates=10
):
    """Hub-based graph-informed split for sparse euclidean data.
    ...
    for _ in range(n_candidates):
        # Select hub pair
        left, right = sparse_select_hub_pair_with_lookup(
            indices, inds, indptr, data, rng_state, in_degree, 16
        )
...
def sparse_angular_hub_split(
    ...
    """Hub-based graph-informed split for sparse angular data.
...
def make_hub_angular_tree(
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = angular_hub_split(data, indices, neighbor_indices, rng_state, n_candidates)
...
def make_hub_bit_tree(
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        ...
        hyperplanes.append(np.array([-1], dtype=np.uint8))
        offsets.append(-np.inf)
        children.append((np.int32(-1), np.int32(-1)))
        point_indices.append(indices)
...
def make_hub_sparse_euclidean_tree(
    inds,
    indptr,
    data,
    indices,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            inds, indptr, data, indices, neighbor_indices, rng_state, n_candidates
        )

        make_hub_sparse_euclidean_tree(
            inds,
            indptr,
            data,
            left_indices,
...
def make_hub_sparse_angular_tree(
    inds,
    indptr,
    data,
    indices,
...
def make_sparse_hub_tree(
    ...
    """Build an RP tree using hub-based hyperplane selection for sparse data.
...
def make_bit_hub_tree(
    ...
    """Build an RP tree using hub-based hyperplane selection for bit-packed data.
...
# ============================================================================
# Simplified Hub Trees (v2) - Precomputed degrees, top-k deterministic selection
# ============================================================================
...
def simple_euclidean_hub_split(
...
def make_simple_hub_euclidean_tree(
    data,
    indices,
    neighbor_indices,
    global_degrees,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = simple_euclidean_hub_split(
            data, indices, neighbor_indices, global_degrees, rng_state
        )

        make_simple_hub_euclidean_tree(
            data,
            left_indices,
            neighbor_indices,
            global_degrees,
...
def make_simple_hub_angular_tree(
    data,
    indices,
    neighbor_indices,
    global_degrees,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = simple_angular_hub_split(
            data, indices, neighbor_indices, global_degrees, rng_state
        )

        make_simple_hub_angular_tree(
            data,
            left_indices,
            neighbor_indices,
            global_degrees,
...
def simple_sparse_euclidean_hub_split(
...
def simple_sparse_angular_hub_split(
    inds, indptr, spdata, indices, neighbor_indices, global_degrees, rng_state
):
...
def make_simple_sparse_hub_euclidean_tree(
    inds,
    indptr,
    spdata,
    indices,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = simple_sparse_euclidean_hub_split(
            inds, indptr, spdata, indices, neighbor_indices, global_degrees, rng_state
        )

        make_simple_sparse_hub_euclidean_tree(
            inds,
            indptr,
            spdata,
            left_indices,
...
def make_simple_sparse_hub_angular_tree(
    inds,
    indptr,
    spdata,
    indices,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = simple_sparse_angular_hub_split(
            inds, indptr, spdata, indices, neighbor_indices, global_degrees, rng_state
        )

        make_simple_sparse_hub_angular_tree(
            inds,
            indptr,
            spdata,
            left_indices,
...
@numba.njit(nogil=True, cache=False)
def make_simple_sparse_hub_tree(
    inds,
    indptr,
    spdata,
    neighbor_indices,
...
def simple_bit_hub_split(data, indices, neighbor_indices, global_degrees, rng_state):
...
def make_simple_bit_hub_tree_recursive(
    data,
    indices,
    neighbor_indices,
    global_degrees,
    ...
    if indices.shape[0] > leaf_size and max_depth > 0:
        (
            ...
            hyperplane,
            offset,
        ) = simple_bit_hub_split(
            data, indices, neighbor_indices, global_degrees, rng_state
        )

        make_simple_bit_hub_tree_recursive(
            data,
            left_indices,
            neighbor_indices,
            global_degrees,
