...
def flagged_heap_push(priorities, indices, flags, p, n):
    """Push a new element onto a heap with flags, without checking for duplicates.

    This is faster than checked_flagged_heap_push when thread safety is handled
    via block-based processing (each thread only updates its own vertex block).
    """
    if p >= priorities[0]:
        return 0
